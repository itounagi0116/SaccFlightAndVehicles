...
public class SaccSyncScript : UdonSharpBehaviour
{
    ...
    private void Update()
    {
        if (IsOwner)
        {
            ...
            {
                if (!Networking.IsClogged || (bool)EngineControl.GetProgramVariable("Piloting"))
                {
                    ...
                    if (!Still || UpdatesSentWhileStill < 2 || (Time.time - UpdateTime > IdleMaxUpdateDelay))
                    {
                        ...
                        O_CurVel = (Vector3)EngineControl.GetProgramVariable("CurrentVel");
    ...
    public override void OnDeserialization()
    {
        if (O_UpdateTime != O_LastUpdateTime && !IsOwner)//only do anything if OnDeserialization was for this script
        {
            ...
            EngineControl.SetProgramVariable("CurrentVel", CurrentVelocity);
